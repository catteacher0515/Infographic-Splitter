...
规则：
1. 中文说明文字必须作为独立元素，例如“一句话”“一堆资料”“一套约束”“自己干活并持续反馈”。
2. 语义上共同表达一个场景的图形应该合并，例如小猫 + 绳子 + 被拉的小猫。
3. 循环图应作为一个整体元素。
4. 序号 + 标题框可以合并为标题元素。
5. 不要把标题框和中文说明文字合并，它们是不同元素。
6. 如果某个候选框本身已经是完整插图或完整中文说明文字，优先保持单独成组。
7. 等号、简单箭头等通用符号可以建议忽略。
8. 不要把整列内容合并成一个大元素。
9. 如果不确定，宁可少合并，不要过度合并。
...
def _is_label_candidate(candidate: dict) -> bool:
    width = int(candidate["width"])
    height = int(candidate["height"])
    return height <= 90 and width >= 80 and width / max(1, height) >= 1.5


def _is_lower_label_candidate(candidate: dict, group_candidates: list[dict]) -> bool:
    if not _is_label_candidate(candidate):
        return False
    ...
    for other in group_candidates:
        if other is candidate:
            continue
        other_y = int(other["y"])
        other_height = int(other["height"])
        if other_height >= 120 and candidate_y >= other_y + other_height:
            return True
    return False
...
def validate_grouping_response(response: dict, candidates: list[dict]) -> dict:
    ...
    valid_candidate_ids = set(candidates_by_id)
    ...
    for fallback_id, group in enumerate(response.get("groups", []), start=1):
        ...
        group_candidates = [candidates_by_id[candidate_id] for candidate_id in candidate_ids]
        label_candidate_ids = [
            candidate_id
            for candidate_id in candidate_ids
            if _is_lower_label_candidate(candidates_by_id[candidate_id], group_candidates)
        ]
        if 0 < len(label_candidate_ids) < len(candidate_ids):
            non_label_candidate_ids = [
                candidate_id
                for candidate_id in candidate_ids
                if candidate_id not in label_candidate_ids
            ]
            groups.append(
                {
                    "id": int(group.get("id") or len(groups) + 1),
                    "file": _clean_group_file(group.get("file"), fallback_id),
                    "candidate_ids": non_label_candidate_ids,
                    "type": group_type,
                    "keep": bool(group.get("keep", True)),
                    "reason": str(group.get("reason", "")),
                }
            )
            for label_id in label_candidate_ids:
                groups.append(
                    {
                        "id": len(groups) + 1,
                        "file": str(candidates_by_id[label_id].get("file") or f"element_{label_id:03d}.png"),
                        "candidate_ids": [label_id],
                        "type": "label",
                        "keep": bool(group.get("keep", True)),
                        "reason": "split from overmerged title group",
                    }
                )
            continue
